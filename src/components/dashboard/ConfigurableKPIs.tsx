...
import { RiskTier, Workflow, WorkflowStatus } from '../../types/workflow.js';
...
export const ConfigurableKPIs: React.FC<ConfigurableKPIsProps> = ({
  ...
  const overdueCount = workflows.filter(
    (workflow) => workflow.status !== 'Retired' && workflow.review_due < today
  ).length;
      ...
      {showConfig && (
        <div className="p-4 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-semibold text-slate-700 dark:text-slate-300 mr-1">Toggle Cards:</span>
            {cards.map((c) => (
              <button
                key={c.id}
                type="button"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3.5">
        ...
        {cards.find((c) => c.id === 'total')?.visible && (
          <button
            ...
            className="text-left bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-blue-400 transition-all shadow-sm cursor-pointer group"
          >
        ...
        {cards.find((c) => c.id === 't4')?.visible && (
          <button
            type="button"
            onClick={() => onFilterTier?.('Tier 4 Prohibited')}
            className="text-left bg-white dark:bg-slate-900 p-4 rounded-xl border border-rose-200 dark:border-rose-900/60 hover:border-rose-500 transition-all shadow-sm cursor-pointer group"
          >
            ...
            <div className="text-2xl font-black text-rose-600 dark:text-rose-400 tabular-nums">{t4Count}</div>
            <div className="text-[10px] text-rose-500/80 font-mono mt-0.5">Requires Intervention</div>
          </button>
        )}
        ...
        {cards.find((c) => c.id === 't3')?.visible && (
          <button
            type="button"
            onClick={() => onFilterTier?.('Tier 3 High')}
            className="text-left bg-white dark:bg-slate-900 p-4 rounded-xl border border-amber-200 dark:border-amber-900/60 hover:border-amber-500 transition-all shadow-sm cursor-pointer group"
          >
            ...
            <div className="text-[10px] text-amber-500/80 font-mono mt-0.5">Legal & Security Review</div>
          </button>
        )}
        ...
        {cards.find((c) => c.id === 'overdue')?.visible && (
          <button
            ...
            className="text-left bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-orange-400 transition-all shadow-sm cursor-pointer group"
          >
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 mb-1">
              <span className="text-[11px] font-medium">Review Overdue</span>
              <Clock className="w-4 h-4 text-orange-500" />
            </div>
            <div className="text-2xl font-black text-orange-500 tabular-nums">{overdueCount}</div>
            <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono mt-0.5">
        ...
        {cards.find((c) => c.id === 'training')?.visible && (
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-purple-400 transition-all shadow-sm">
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 mb-1">
        ...
        {cards.find((c) => c.id === 'shadow')?.visible && (
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-slate-400 transition-all shadow-sm">
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 mb-1">
              <span className="text-[11px] font-medium">Shadow IT Gap</span>
              <EyeOff className="w-4 h-4 text-slate-400" />
            </div>
            <div className="text-2xl font-black text-slate-700 dark:text-slate-300 tabular-nums">~142*</div>
