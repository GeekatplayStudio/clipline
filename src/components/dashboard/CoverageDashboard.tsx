    ...
    <div className="space-y-6 max-w-7xl mx-auto py-2">
      ...
      {dashboardTab === 'analytics' ? (
        ...
        <div className="bg-white border border-slate-200 rounded p-4 shadow-sm">
          ...
          <div className="mt-2 flex items-baseline space-x-2">
            <span className={`text-3xl font-extrabold font-mono ${metrics.overdueReviewsCount > 0 ? 'text-rose-700' : 'text-slate-900'}`}>
              {metrics.overdueReviewsCount}
            </span>
            <span className="text-xs text-slate-600 font-medium">
              Overdue for Re-review
            </span>
          </div>
          <p className="text-[11px] text-slate-500 mt-2">
            ...
            return (
              <div key={lit.lob} className="text-xs">
                <div className="flex justify-between mb-1">
                  ...
                  <span className={`font-mono text-[11px] font-bold ${meetsTarget ? 'text-emerald-700' : 'text-amber-700'}`}>
