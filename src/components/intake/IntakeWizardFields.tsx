...
interface IntakeWizardFieldsProps {
  step: number;
  error: string | null;
  formData: WorkflowIntakeFormData;
  setFormData: React.Dispatch<React.SetStateAction<WorkflowIntakeFormData>>;
}

export const IntakeWizardFields: React.FC<IntakeWizardFieldsProps> = ({
  step,
  error,
  formData,
  setFormData,
  ...
  const handleToolToggle = (tool: ToolName) => {
    setFormData((prev) => {
      ...
      const updated = exists ? prev.tools_used.filter((t) => t !== tool) : [...prev.tools_used, tool];
  ...
  return (
    <>
      ...
      {step === 1 && (
        <div className="space-y-4 text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block font-semibold text-slate-800 mb-1">Owner Role</label>
              <input
                type="text"
          ...
          <div>
            <label className="block font-semibold text-slate-800 mb-1">Builder AI Literacy Tier</label>
            <select
              value={formData.builder_tier}
              onChange={(e) =>
                setFormData({ ...formData, builder_tier: e.target.value as BuilderLiteracyTier })
              }
              className="w-full border border-slate-300 rounded px-3 py-2 text-xs focus:ring-1 focus:ring-slate-900 focus:outline-none bg-white"
            >
      ...
      {step === 2 && (
        <div className="space-y-4 text-xs">
          <div>
            <h3 className="font-semibold text-slate-900 text-sm">What data does this workflow touch?</h3>
        ...
        <div className="space-y-4 text-xs">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, decision_influence: e.target.value as DecisionInfluence })
              }
              className="w-full border border-slate-300 rounded px-3 py-2 text-xs focus:ring-1 focus:ring-slate-900 focus:outline-none bg-white"
            >
          ...
          <div>
            <label className="block font-semibold text-slate-800 mb-1">Output Audience</label>
            <select
              value={formData.output_audience}
              onChange={(e) =>
                setFormData({ ...formData, output_audience: e.target.value as OutputAudience })
              }
              className="w-full border border-slate-300 rounded px-3 py-2 text-xs focus:ring-1 focus:ring-slate-900 focus:outline-none bg-white"
            >
              {AUDIENCES.map((aud) => (
          ...
          <div>
            <label className="block font-semibold text-slate-800 mb-1">Human Review Oversight</label>
            <select
              value={formData.human_review}
              onChange={(e) =>
                setFormData({ ...formData, human_review: e.target.value as HumanReviewFrequency })
              }
              className="w-full border border-slate-300 rounded px-3 py-2 text-xs focus:ring-1 focus:ring-slate-900 focus:outline-none bg-white"
            >
            ...
            <label className="flex items-start space-x-2.5 cursor-pointer">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500">
                  Check if you send prompts to non-enterprise APIs, personal consumer accounts, or unapproved
                  third-party servers.
                </p>
              </div>
            </label>
      ...
      {step === 4 && (
        <div className="space-y-5 text-xs">
          ...
          <div>
            <label className="block font-semibold text-slate-800 mb-1">Build Architecture Type</label>
            <select
          ...
          <div className="border border-slate-300 rounded p-4 bg-slate-50 space-y-3">
            ...
            <div>
              <p className="text-slate-800 font-medium leading-relaxed">{liveEvaluation.reason}</p>
            </div>
            ...
            <div className="pt-2 border-t border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
              ...
              <div>
                ...
                <span className="font-semibold text-slate-800">
                  Every {liveEvaluation.reviewCadenceMonths} Months
                </span>
              </div>
            </div>
          </div>
